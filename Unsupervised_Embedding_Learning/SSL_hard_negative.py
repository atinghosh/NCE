...
import time
import numpy as np
from Unsupervised_Embedding_Learning import models
from Unsupervised_Embedding_Learning import datasets
import math

from Unsupervised_Embedding_Learning.BatchAverage import BatchCriterion
from Unsupervised_Embedding_Learning.utils import *
from tensorboardX import SummaryWriter

# Labels = "/data02/Atin/mean-teacher/pytorch/data-local/labels/cifar10/250_balanced_labels/01.txt"
...
labels_index_list = return_train_labels_index(Labels)
...
trainset_labeled = datasets.CIFAR10InstanceLabelled(labels_index_list, root='./data', train=True, download=True,
                                                    transform=transform_train)
labeled_dataloader = torch.utils.data.DataLoader(trainset_labeled, batch_size=50, shuffle=True, num_workers=0)
...
def train(epoch):
    ...
    for batch_idx, (inputs1, inputs2, _, indexes) in enumerate(trainloader):
        ...
        # factor = loss.item()/sup_loss.item()
        # if epoch == 0:
        #     factor = 1
        # elif epoch == 1:
        ...
        #         factor = loss_ema.item() / sup_loss_ema.item()
        factor = 1
...
for epoch in range(start_epoch, start_epoch + 301):
    ...
    if epoch % test_epoch == 0:
        ...
        if acc > best_acc:
            ...
            torch.save(state, args.model_dir + suffix + '_best_working.t')
            best_acc = acc
